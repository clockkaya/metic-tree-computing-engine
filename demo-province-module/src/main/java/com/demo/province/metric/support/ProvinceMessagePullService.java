...
import com.sama.api.ledger.bean.GlobalMetricResultDO;
import com.sama.api.ledger.bean.GroupStatisticBakDO;
import com.sama.api.ledger.bean.MetricResultCopyDO;
import com.sama.api.ledger.bean.indicator.MetricConstants;
import com.sama.ledger.config.NacosConfig;
import com.sama.ledger.service.GlobalMetricResultService;
import com.sama.ledger.service.GroupStatisticBakService;
import com.sama.ledger.service.MetricResultCopyService;
 ...
 * 省侧消息拉取
 * @author: huxh
 * @description: 详细参见《交互流程》中 Group -> Province 时，省侧作为拉取端的处理（此处实际消费集团侧 Kafka）
 * @datetime: 2025/8/14 11:06
 */
@Service
@Order(-1)
public class ProvinceMessagePullService implements ApplicationRunner {

    private static final Logger logger = LogManager.getLogger(ProvinceMessagePullService.class);

    private static final String LOG_HEADER = "【省侧|拉取端】 ";
    ...
    @Resource
    GlobalMetricResultService  globalMetricResultService;

    @Resource
    MetricResultCopyService metricResultCopyService;
    ...
    protected void init() {
        ...
        topicMetricAvg = nacosConfig.getTopicMetricAvg();
        topicMetricResultCopy = nacosConfig.getTopicMetricResultCopy();
        groupConsumer.subscribe(Arrays.asList(MetricConstants.KafkaTopic.GROUP_STATISTIC, topicMetricAvg, topicMetricResultCopy));

        // 定义静态的加载语句
        loadConditionalStatements.put(MetricConstants.KafkaTopic.GROUP_STATISTIC, this::loadGroupStatisticBak);
        loadConditionalStatements.put(topicMetricAvg, this::loadGlobalMetricResult);
        loadConditionalStatements.put(topicMetricResultCopy, this::loadMetricResultCopy);
    }

    private void routeTransform(String topic, String key, String value, ConcurrentHashMap<String, List<Object>> dataContainer) {
        if (topic.startsWith(MetricConstants.KafkaTopic.GROUP_STATISTIC)) {
            transformGroupStatisticBak(dataContainer, value);
        }
        if(topic.startsWith(topicMetricAvg)){
            transformGlobalMetricResult(dataContainer, value);
        }
    }

    private void transformGroupStatisticBak(ConcurrentHashMap<String, List<Object>> dataContainer, String value){
        // 此处是 List
        List<GroupStatisticBakDO> dataList = JSON.parseObject(value, new TypeReference<ArrayList<GroupStatisticBakDO>>(){});
        dataContainer.computeIfAbsent(MetricConstants.KafkaTopic.GROUP_STATISTIC, k -> new CopyOnWriteArrayList<>()).addAll(dataList);
    }

    private void loadGroupStatisticBak(List<?> dataList){
        groupStatisticBakService.insertBatch((List<GroupStatisticBakDO>) dataList);
        logger.info(LOG_HEADER + "计入表 group_statistic_bak 成功，共计 {} 条", dataList.size());
    }

    private void transformGlobalMetricResult(ConcurrentHashMap<String, List<Object>> dataContainer, String value){
        // 此处是 List
        List<GlobalMetricResultDO> dataList = JSON.parseObject(value, new TypeReference<ArrayList<GlobalMetricResultDO>>(){});
        dataContainer.computeIfAbsent(topicMetricAvg, k -> new CopyOnWriteArrayList<>()).addAll(dataList);
    }

    private void loadGlobalMetricResult(List<?> dataList){
        globalMetricResultService.insertBatch((List<GlobalMetricResultDO>) dataList);
        logger.info(LOG_HEADER + "计入表 global_metric_result 成功，共计 {} 条", dataList.size());
    }

    private void loadMetricResultCopy(List<?> dataList){
        metricResultCopyService.insertBatch((List<MetricResultCopyDO>) dataList);
        logger.info(LOG_HEADER + "计入表 metric_result_copy 成功，共计 {} 条", dataList.size());
    }
}
